...
const config = {
  PREFIX: "..",
  token: "",
  geniusapi: 'fhH5zbkEm_O5LlP7W2zd1HRvyfjdcvaikGZk3107h5k5iwIfO4sceKDHMgsmqshI'
}
...
client.on("message", async message => {
  ...
  try {
    ...
    if (command === "invite" || command === "add") {
      return embedbuilder(client, message, "#fffff0", "Invite me!", "[`Click here`](https://discord.com/api/oauth2/authorize?client_id=828088821680701480&permissions=0&scope=bot)   |   [`Website`](https://msha.ke/promusic/#top-picks)   |   :heart: Thanks for inviting!")
    }
    if (command === "radio") {
    ...
    if (command === "help" || command === "about" || command === "h" || command === "info") {
      let helpembed = new Discord.MessageEmbed()
        ...
        .setURL("https://msha.ke/promusic/#top-picks")
        .setAuthor(message.author.tag, message.member.user.displayAvatarURL({ dynamic: true }), "https://msha.ke/promusic/#top-picks")
        ...
        **Prefix:** \`${prefix}\`   *change with:* \`${prefix}prefix <NEW PREFIX>\`
        ...
        \`Youtube\`, \`Soundcloud\`, [\`More\`](https://msha.ke/promusic/#top-picks), ...
        `)
        .addField("***BOT BY:***", `
        <@723096752877469787> \`\`[\`Website\`](https://msha.ke/promusic/#top-picks)
        `)
        .addField("***SUPPORT:***", `
        [\`Server\`](https://discord.gg/vAHadtbF63) | [\`Pro Music - Website\`](https://msha.ke/promusic/#top-picks) | [\`Invite\`](https://discord.com/api/oauth2/authorize?client_id=828088821680701480&permissions=0&scope=bot) | [\`Vote\`](https://top.gg/bot/)
        `)
    ...
    else if (command === "search" || command == "play" || command == "p" || command == "sc") {
     ...
     //  }
      ...
      embedbuilder(client, message, "RED", "STOPPED!", `STAYING IN THE CHANNEL`).then(msg => msg.delete({ timeout: 5000 }).catch(console.error))
      try {
        await message.guild.channels.cache.get(db.get(`playingchannel_${message.guild.id}`)).messages.cache.get(db.get(`playingembed_${message.guild.id}`), false, true).delete().catch(console.error);
      } catch (error) {
        console.error(error)
...
distube
  ...
  .on("addList", (message, queue, playlist, song) => {
    try {
      return embedbuilder(client, message, "#fffff0", "Added a Playling!", `Playlist: [\`${playlist.name}\`](${playlist.url})  -  \`${playlist.songs.length} songs\` \n\nRequested by: ${song.user}`, playlist.thumbnail)
    } catch (error) {
      console.error
    }
  })
  ...
  .on("empty", message => {
    ...
    try {
      return embedbuilder(client, message, "RED", "MUSIC ENDED STAYING IN CHANNEL!").then(msg => msg.delete({ timeout: 5000 }).catch(console.error))
    } catch (error) {
      console.error
    }
  })
    ...
    collector.on("collect", async (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        ...
        case "⏹":
          ...
          playingMessage.reactions.removeAll().catch(console.error);
          playingMessage.delete({ timeout: client.ws.ping }).catch(console.error);
          embedbuilder(client, message, "RED", "STOPPED!", `STAYING IN THE CHANNEL`).then(msg => msg.delete({ timeout: 3000 }).catch(console.error))
          break;
